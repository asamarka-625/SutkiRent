# Внешние зависимости
from sqladmin import ModelView
# Внутренние модули
from models import Parking


# Админка для Parking
class ParkingAdmin(ModelView, model=Parking):
    column_list = [
        Parking.id,
        Parking.title
    ]

    column_labels = {
        Parking.id: "Идентификатор",
        Parking.title: "Название",
        Parking.created_at: "Дата создания",
        Parking.apartments: "Объекты"
    }

    column_searchable_list = [Parking.id] # список столбцов, которые можно искать
    column_sortable_list = [
        Parking.id
    ]  # список столбцов, которые можно сортировать

    column_default_sort = [(Parking.id, True)]

    form_create_rules = [
        "title"
    ]

    column_details_list = [
        Parking.id,
        Parking.title,
        Parking.created_at,
        Parking.apartments
    ]

    form_edit_rules = [
        "title"
    ]

    can_create = True # право создавать
    can_edit = True # право редактировать
    can_delete = True # право удалять
    can_view_details = True # право смотреть всю информацию
    can_export = True # право экспортировать

    name = "Парковка" # название
    name_plural = "Парковки" # множественное название
    icon = "fa-solid fa-layer-group" # иконка
    category = "Объекты" # категория
    category_icon = "fa-solid fa-list" # иконка категории

    page_size = 10
    page_size_options = [10, 25, 50, 100]