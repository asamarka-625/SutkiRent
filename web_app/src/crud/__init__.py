from web_app.src.crud.apartment import (sql_get_available_apartments, sql_upd_region_for_apartments,
                                        sql_add_favorite_for_user, sql_remove_favorite_for_user,
                                        sql_get_apartment_by_id, sql_get_data_for_filters)
from web_app.src.crud.content import sql_get_contents_by_category, sql_get_content_by_id
from web_app.src.crud.region import sql_get_regions
from web_app.src.crud.user import (sql_get_user_by_email, sql_get_user_by_id, sql_create_user,
                                   sql_update_password_user_by_email, sql_update_user_info,
                                   sql_check_user_has_apartments, sql_check_user_has_apartment_by_id,
                                   sql_get_user_apartments)
from web_app.src.crud.inventory import (sql_get_inventory_by_apartment_id, sql_get_brands, sql_get_items,
                                        sql_update_inventory_by_apartment_id)